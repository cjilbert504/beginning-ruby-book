...
class Bot
  ...
  def greeting
    _random_response :greeting
  end

  # Returns a random farewell message as specified in the bot's
  # data file
  def farewell
    _random_response :farewell
  end

  # Responds to input text as given by a user
  def response_to(input)
    prepared_input = _preprocess(input.downcase)
    sentence = _best_sentence(prepared_input)
    reversed_sentence = WordPlay.switch_pronouns(sentence)
    responses = _possible_responses(sentence)
  ...
  # used as responses
  def _possible_responses(sentence)
    responses = []
    ...
    @data[:responses].keys.each do |pattern|
      ...
      if sentence.match("\b" + pattern.gsub(/\*/, "") + "\b")
        ...
        # If the pattern contains substitiution placeholders,
